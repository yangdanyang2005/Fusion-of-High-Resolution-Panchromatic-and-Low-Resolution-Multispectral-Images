...
subplot(2, 3, 1); imshow(uint8(ImageP), []); title('(1)Original Pan'); % 显示原始的全色图像
% 保存原始的全色图像
for fmt = save_formats
    imwrite(uint8(ImageP), [output_dir, 'Original_Pan.' fmt{1}]); 
end

subplot(2, 3, 4); imshow(uint8(P), []); title('(4)Registered Pan by SIRF'); % 显示注册后的全色图像
% 保存注册后的全色图像
for fmt = save_formats
    imwrite(uint8(P), [output_dir, 'Registered_Pan_by_SIRF.' fmt{1}]); 
end

subplot(2, 3, 2); imshow(uint8(ImageMS(:,:,1:3)), []); title('(2)Original RGB'); % 显示原始 RGB 图像
% 保存原始 RGB 图像
for fmt = save_formats
    imwrite(uint8(ImageMS(:,:,1:3)), [output_dir, 'Original_RGB.' fmt{1}]); 
end

subplot(2, 3, 5); imshow(uint8(im(:,:,1:3)), []); title('(5)Fusion by SIRF (RGB)'); 
% 保存融合后的 RGB 图像
for fmt = save_formats
    imwrite(uint8(im(:,:,1:3)), [output_dir, 'Fusion_RGB.' fmt{1}]); 
end

subplot(2, 3, 3); imshow(uint8(ImageMS(:,:,4)), []); title('(3)Original Nir'); % 显示原始近红外图像
% 保存原始近红外图像
for fmt = save_formats
    imwrite(uint8(ImageMS(:,:,4)), [output_dir, 'Original_Nir.' fmt{1}]); 
end

subplot(2, 3, 6); imshow(uint8(im(:,:,4)), []); title('(6)Fusion by SIRF (Nir)'); % 显示融合后的近红外图像
% 保存融合后的近红外图像
for fmt = save_formats
